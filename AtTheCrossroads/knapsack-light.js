...
function solution(value1, weight1, value2, weight2, maxW) {
    ...
    // returns if all both weights are larger than max weight
    if (maxW < weight1 && maxW < weight2) return 0;

    // returns value if total weight is less than or equal to max weight
    if (maxW >= weight1 + weight2)return value1 + value2;

    // returns value of the largest weight if the weight of additional will be greater than max weight
